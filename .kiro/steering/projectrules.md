...
9. 请注意，api 接口部分应保持清爽、干净的代码风格，除参数预处理、调用 service 方法、返回返回值、处理返回体之外，不应当有任何业务逻辑的方法和代码。最佳 api 接口实践：.\app\api\admin\users.py 以及 .\app\api\admin\api_logs.py 如果你需要修改 api 或者创建 api 模块时可以供你参考。
...
11. 已配置自动加载表单功能，如果需要修改xx数据库表，我会提前删除数据库中的表，然后你修改表单后我启动项目即可自动添加表单。
